...
ログインフォームに入力された値からパスワードを比較し、認証する。<br>

`User`, `Owner`, `Admin` 3 つのフォームがあるので`routeIs()`でルートを確認しつつ`Auth::guard()`を追加<br>

`Auth::guard($guard)->attempt(以下略`<br>

- 参考: https://readouble.com/laravel/8.x/ja/authentication.html (attempt)<br>

### ハンズオン

`app/Http/Requests/Auth/LoginRequest.php`を編集<br>
