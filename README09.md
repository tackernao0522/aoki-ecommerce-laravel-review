...
前回は、<br>
namespace をフォルダ構成と合わせる<br>

`view('login') -> view('onwer.login') (フォルダ名.ファイル名)`<br>

```
RouteServiceProvider::HOME -> RouteServiceProvider::OWNER_HOME
```

`Auth::guard('web')->logout() -> Auth::guard('owners')->logout()`<br>

### ビュー

`views/user`フォルダを作成<br>
auth フォルダを`views/user/auth`フォルダに変更<br>

このフォルダごとコピーし<br>
`views/owner/auth`と`views/admin/auth`を作成<br>

合わせて welcome, dashboard もコピーする<br>

### 7-2-2. ビュー

`RouteServiceProvider`で<br>
prefix と as で設定したように route 内を変更<br>

`route('login')->route('user.login')`<br>

### ハンズオン

- `resources/views/user`ディレクトリを作成<br>

* `resources/views/user`ディレクトリに`resources/views/auth`ディレクトリごと移動<br>

- `resources/views/owner`ディレクトリを作成<br>

- `$ cp -r resources/views/user/auth resources/views/owner/auth`を実行<br>

* `resources/views/admin`ディレクトリを作成<br>

- `$ cp -r resources/views/user/auth resources/views/admin/auth`を実行<br>

* `resources/views/admin`と`resources/views/user`と`resources/views/owner`<br>
  の各ディレクトリに`resources/views/dashboard.blade.php`と`resources/views/welcome.blade.php`をコピー<br>

- `resources/views/dashboard.blade.php`と`resources/views/welcome.blade.php`は削除する<br>

* `resources/views/user/welcome.blade.php`を編集<br>
  ...
  <body class="antialiased">
    ...
    >
      ...
      <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
        ...
        @if (Route::has('user.register')) <!-- 編集 -->
        <a
          href="{{ route('user.register') }}" // 編集
...
- `resources/views/user/auth/confirm-password.blade.php`を編集<br>

```html:confirm-password.blade.php
<x-guest-layout>
  <x-auth-card>
    <x-slot name="logo">
      <a href="/">
    ...
    <div class="mb-4 text-sm text-gray-600">
      {{ __('This is a secure area of the application. Please confirm your
...
- `resorces/views/user/auth/forgot-password.blade.php`を編集<br>

```html:forgot-password.blade.php
<x-guest-layout>
  <x-auth-card>
    <x-slot name="logo">
      <a href="/">
...
- `resources/views/user/auth/login.blade.php`を編集<br>
...
<x-guest-layout>
  <x-auth-card>
    ...
    <form method="POST" action="{{ route('user.login') }}"> <!-- 編集 -->
      @csrf
      ...
      <div class="flex items-center justify-end mt-4">
        @if (Route::has('user.password.request')) <!-- 編集 -->
        <a
          class="underline text-sm text-gray-600 hover:text-gray-900"
          href="{{ route('user.password.request') }}" // 編集
        >
...
- `resources/views/user/auth/register.blade.php`を編集<br>
...
- `resources/views/user/auth/reset-password.blade.php`を編集<br>

```html:reset-password.blade.php
<x-guest-layout>
  <x-auth-card>
    <x-slot name="logo">
      <a href="/">
...
- `resources/views/user/auth/verify-email.blade.php`を編集<br>

```html:verify-email.blade.php
<x-guest-layout>
  <x-auth-card>
    <x-slot name="logo">
      <a href="/">
    ...
    <div class="mb-4 text-sm text-gray-600">
      {{ __('Thanks for signing up! Before getting started, could you verify
      your email address by clicking on the link we just emailed to you? If you
      didn\'t receive the email, we will gladly send you another.') }}
    </div>

    @if (session('status') == 'verification-link-sent')
    <div class="mb-4 font-medium text-sm text-green-600">
      {{ __('A new verification link has been sent to the email address you
